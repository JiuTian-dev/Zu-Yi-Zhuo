...
export class Reveal
{
    ...
    updateStep(step)
    {
            ...
            this.game.world.intro.circle.hide(() =>
            {
                ...
                else
                {
                    ...
                    const next = () =>
                    {
                        ...
                        // Same original reveal animation (embedded or standalone).
                        this.updateStep(1)
                    }
        ...
        else if(step === 2)
        {
            ...
            // Embedded 组一桌: hide landing letters only; show road dressing after intro.
            // Do not cull the world — asset placement fills city/transition/nature bands.
            // Intro vehicle / lamp / sign / parking bay stay as ZuoyizhuoLot set them.
            if(window.parent && window.parent !== window)
            {
                this.game.world.hideLandingLetters?.()
                this.game.world.zuoyizhuoDressing?.reveal?.()
            }

            // Tower only after leaving the start/intro screen.
            this.game.world.zuoyizhuoLot?.replaceTreesWithBuildings?.()

            // Ensure tree leaves are fully visible after intro see-through fade.
            for(const trees of [
            ...
            ])
            {
                if(trees?.leaves)
                {
                    trees.leaves.seeThroughMultiplier = 1
                    if(trees.leaves.mesh)
                        trees.leaves.mesh.visible = true
                }
                if(trees?.bodies)
                    trees.bodies.visible = true
            }
