...
export class Area
{
    ...
    /**
     * Hide all area props + disable physics (Bruno portfolio set dressing).
     * Frustum will not re-show them.
     */
    strip()
    {
        this.stripped = true

        // Hide frustum-managed extras (e.g. achievements tiles) before clearing the list.
        for(const object3D of this.objects.hideable)
        {
            if(!object3D)
                continue
        ...
        const hidePoint = (point) =>
        {
            if(!point)
                return
            point.permanentlyHidden = true
            if(typeof point.hide === 'function')
                point.hide()
            if(point.intersect)
                point.intersect.active = false
            point.recoveryState = 3 // InteractivePoints.STATE_HIDDEN
            point.showAfterReveal = false
        }

        // Common single / multi interactive-point fields on area subclasses.
        hidePoint(this.interactivePoint)
        if(Array.isArray(this.interactivePoints))
        {
            for(const point of this.interactivePoints)
                hidePoint(point)
        }

        // Social / other areas may stash points under arbitrary keys.
        for(const key of Object.keys(this))
        {
            const value = this[key]
            if(value && value !== this.interactivePoint && typeof value.hide === 'function' && value.intersect)
                hidePoint(value)
        }

        // Hide every reference mesh (covers Lab/Projects image planes still parented in refs).
        this.references?.items?.forEach((list) =>
        {
