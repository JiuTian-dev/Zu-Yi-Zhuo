...
export class LabArea extends Area
{
    ...
    forceHideLabUi()
    {
        ...
        hideDeep(this.images?.mesh)
        hideDeep(this.title?.group)
        hideDeep(this.url?.group)
        hideDeep(this.adjacents?.previous?.group)
        hideDeep(this.adjacents?.next?.group)
        hideDeep(this.blackBoard?.group)

        if(this.scroller?.minis?.items)
        {
            for(const mini of this.scroller.minis.items)
            {
                hideDeep(mini.group)
    ...
    /**
     * Lab rebuilds mini cards + main image asynchronously after Area.strip();
     * keep forcing them off and disable scroller/image callbacks.
     */
    strip()
    {
        super.strip()
        ...
        if(this.adjacents?.previous?.intersect)
            this.adjacents.previous.intersect.active = false
        if(this.adjacents?.next?.intersect)
            this.adjacents.next.intersect.active = false
        if(this.url?.intersect)
            this.url.intersect.active = false

        this.forceHideLabUi()
        // Texture loads finish after strip — hide again a few frames later.
        this.game.ticker.wait(15, () => this.forceHideLabUi())
        this.game.ticker.wait(60, () => this.forceHideLabUi())
        this.game.ticker.wait(180, () => this.forceHideLabUi())
    }
}