...
export class Trees
{
    ...
    hideWhere(testPos)
    {
        if(!this.bodies || !this.references)
            return

        const hidden = new THREE.Matrix4().makeScale(0, 0, 0)
        hidden.setPosition(0, 200, 0)
        const leafCount = this.modelParts.leaves.length

        for(let i = 0; i < this.references.length; i++)
        {
            const pos = this.references[i].position
            if(!testPos(pos))
                continue

            this.bodies.setMatrixAt(i, hidden)
            if(this.leaves?.instanceMatrix)
            {
                for(let l = 0; l < leafCount; l++)
                    hidden.toArray(this.leaves.instanceMatrix.array, (i * leafCount + l) * 16)
            }
        }
