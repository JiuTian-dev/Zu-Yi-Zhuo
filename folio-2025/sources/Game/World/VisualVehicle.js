...
export class VisualVehicle
{
    ...
    setParts()
    {
        ...
        // Custom GLB (e.g. Vehicle_SUV) has no Bruno part names — wrap whole model as chassis.
        if(!this.parts.chassis)
        {
            this.customSimple = true
            const wrapper = new THREE.Group()
            wrapper.name = 'chassis'
            wrapper.rotation.reorder('YXZ')
            // SUV: face opposite of default export; nudge laterally (vehicle +Z = right).
            this.model.rotation.y = 0
            // Normalize size so SUV roughly matches default car footprint (~3.2m).
            this.model.updateMatrixWorld(true)
            const box = new THREE.Box3().setFromObject(this.model)
            const size = new THREE.Vector3()
            box.getSize(size)
            const longest = Math.max(size.x, size.z, 0.001)
            const scale = 3.2 / longest
            this.model.scale.setScalar(scale)
            // Center on reveal circle (no lateral nudge); seat wheels on local y=0.
            this.model.position.set(0, 0, 0)
            this.model.updateMatrixWorld(true)
            const grounded = new THREE.Box3().setFromObject(this.model)
            this.model.position.y = -grounded.min.y
            // Physics body sits above ground; visual chassis will force y=0 in update().
            this.customSimpleGroundY = 0
            wrapper.add(this.model)
            this.parts.chassis = wrapper
        ...
        // Wheel
        if(this.parts.wheelContainer)
            this.game.materials.updateObject(this.parts.wheelContainer)
    }

    setPaints()
    {
        if(this.customSimple || !this.parts.bodyPainted)
            return
    ...
    setWheels()
    {
        ...
        if(this.customSimple || !this.parts.wheelContainer)
            return
    ...
    update()
    {
        ...
        // Custom simple vehicle: chassis follow only (no Bruno wheel rig).
        if(this.customSimple)
        {
            // Keep SUV on the ground — physics body y is chassis height, not wheel contact.
            this.parts.chassis.position.y = this.customSimpleGroundY ?? 0
            this.mainGroundTrack.update(physicalVehicle.position, physicalVehicle.position.y < 1.5)
            if(this.boostTrails?.leftReference && this.boostTrails?.left)
                this.boostTrails.leftReference.getWorldPosition(this.boostTrails.left.position)
            if(this.boostTrails?.rightReference && this.boostTrails?.right)
                this.boostTrails.rightReference.getWorldPosition(this.boostTrails.right.position)
            const projected = this.parts.chassis.position.clone().project(this.game.view.camera)
            this.screenPosition.set(projected.x * 0.5 + 0.5, projected.y * 0.5 + 0.5)
            return
        }
