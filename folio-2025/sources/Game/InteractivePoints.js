...
export class InteractivePoints
{
    ...
    constructor()
    {
        ...
        // Designed at ~1080p; scale diamonds so on-screen size tracks viewport.
        this.baseGroupScale = 0.85
        this.referenceViewportHeight = 1080
    ...
    getAdaptiveGroupScale()
    {
        const height = Math.max(this.game.viewport?.height || this.referenceViewportHeight, 320)
        const pixelBoost = Math.min(Math.max(this.game.viewport?.pixelRatio || 1, 1), 2)
        // Taller viewports → slightly smaller world scale so markers stay similar CSS-pixel size.
        return this.baseGroupScale * (this.referenceViewportHeight / height) * (1.15 / (0.85 + pixelBoost * 0.15))
    }

    applyAdaptiveScaleToAll()
    {
        const scale = this.getAdaptiveGroupScale()
    ...
    )
    {
        ...
        group.scale.setScalar(this.getAdaptiveGroupScale())
