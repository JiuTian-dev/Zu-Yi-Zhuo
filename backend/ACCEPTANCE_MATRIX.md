...
当前基线为 **424 passed**。最近一个后端功能切片是 D129（GROUND 卡保留授权内容信号 ID）；对应实现提交为 `8dd07d3`，设计提交为 `0b2fa1f`。
