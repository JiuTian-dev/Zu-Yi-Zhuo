...
当前基线为 **416 passed**。最近一个后端功能切片是 D126（GROUND 资料卡进入干预回放账本）；对应实现提交为 `735b398`，设计提交为 `58fffba`。
