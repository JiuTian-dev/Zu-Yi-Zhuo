...
- `POST /intents/preview`：接收用户主动提出的一段自然语言需求，返回 `clarify`、`join_existing` 或 `new_table` 路由和最多 5 个有空席的公开桌候选（含 `available_seats`）；不读取个人 source、不自动建桌或入席。
...
- `GET /tables/{table_id}/replay`：返回原始真人消息和状态快照，并附带公开的 `interventions`、`comments`、`comment_promotions` 账本和已保存的 `source_signals`，重连时可直接恢复整桌叙事。
...
- `GET /tables/{table_id}/evaluation?participant_id=...`：桌内成员查看当前单桌的隐私安全评估指标（真人轮次、主持介入/反思、行动回响和匿名价值反馈完成度）；只读派生，不返回个人评分、备注或行为事件明细。
