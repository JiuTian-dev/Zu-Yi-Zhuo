...
评委先在湖边登录页一键选择自己的 3D 角色，再从首页点击“先让 Agent 认识我”，通过两轮自由聊天生成并确认可编辑标签。系统随后解释为什么由三位互补桌友入席，并用 SUV 找桌动画进入主题为“离开大城市，是逃避还是重新选择生活？”的圆桌。一桌包含评委、三位明确标注的模拟讨论者（县城医生林夏、产品工程师周砚、城市研究者程野）和一位圆桌主持；用户发言后，桌友会根据这句新内容实时接话，收桌后生成讨论总结、个人贡献、新问题和值得继续认识的桌友。模拟身份仅用于无人在线时保证评审可重复体验，不代表知乎真实用户。
...
- 当前评委路径统一使用瑞士湖边视觉、原创角色和 3D 圆桌空间；登录、画像、候选、找桌、圆桌与收桌卡保持同一套低压视觉语言。
- SUV 被保留为“Agent 带你找桌”的产品化转场，沿地图路线驶向最佳匹配桌；用户可直接抵达，不把驾驶做成额外玩法。
...
- [x] 完成一键角色登录；用户选中的角色贯穿画像、圆桌和收桌卡，三位预置桌友自动使用其余角色且不重复。
- [x] 完成自由聊天画像、可编辑/可隐藏标签、可解释互补匹配和 SUV 自动找桌转场。
- [x] 完成新主题圆桌、用户发言触发桌友实时回复、显式收桌进度、失败恢复与社交关系回流。
