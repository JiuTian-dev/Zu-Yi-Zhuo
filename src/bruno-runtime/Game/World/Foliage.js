...
export class Foliage
{
    ...
    setMaterial()
    {
        ...
        this.material.focusScreenPosition = uniform(vec2())
        ...
        const alphaNode = Fn(() =>
        {
            ...
            // Keep the same soft focus clearing used by the original foliage material.
            if(this.seeThrough)
            {
                const toFocus = screenUV.sub(this.material.focusScreenPosition)
                toFocus.mulAssign(vec2(screenSize.x.div(screenSize.y), 1))
                const distanceToFocus = toFocus.length()
                const distanceFade = smoothstep(this.material.seeThroughEdgeMin, this.material.seeThroughEdgeMax, distanceToFocus)
    ...
    update()
    {
        // Keep the original foliage material and center its soft clearing on
        // the frame, independent of any product state.
        if(this.seeThrough)
            this.material.focusScreenPosition.value.set(0.5, 0.5)
