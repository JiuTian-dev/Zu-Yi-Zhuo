...
export type TableSummary = TableSummaryBase & {
  status: TableStatus
  entryMode: TableEntryMode
  transitionPreset: TableTransitionPreset
}

type TableState = Pick<TableSummary, 'status' | 'entryMode' | 'transitionPreset'>
...
): TableSummary => ({ id, worldId, hook, seatedCount, missingPerspective, ...state, ...extras })

const formingTable: TableState = { status: 'forming', entryMode: 'preview', transitionPreset: 'cover-only' }
const liveTable: TableState = { status: 'live', entryMode: 'immersive', transitionPreset: 'valley' }
...
export const worlds: WorldSummary[] = [
  {
    ...
    accent: '#FF9B42',
    tables: [
      table('leaving-the-city', 'campfire', '关于离开大城市这件事，他们已经聊了三天。', 4, '还缺一个真正离开过的人', formingTable, {
        recommendedBecause: '你一直在关注「要不要离开大城市」，而这桌聊了三天，还没有一个真正离开过的人。',
      }),
      table('staying-in-the-city', 'campfire', '留在大城市，真的值得吗？', 3, '还缺一个决定留下来的人', formingTable),
      table('starting-again', 'campfire', '三十岁以后，重新开始意味着什么？', 2, '还缺一个已经重新开始的人', formingTable),
    ],
  },
  {
    id: 'valley',
    ...
    accent: '#A8CE84',
    tables: [
      table('first-solo-trip', 'valley', '第一次一个人旅行。', 3, '还缺一个刚刚独自出发的人', formingTable),
      table('learning-to-rest', 'valley', '为什么我们越来越不会休息？', 4, '还缺一个真正慢下来的人', liveTable, {
      ...
      table('worth-the-trip', 'valley', '有哪些值得专程去吃的地方？', 2, '还缺一个会为味道出发的人', formingTable),
    ],
  },
  {
    id: 'workshop',
    ...
    accent: '#FFD28A',
    tables: [
      table('learning-to-code', 'workshop', 'AI 时代还要学编程吗？', 4, '还缺一个刚开始学习的人', formingTable, {
        recommendedBecause: '你最近收藏过 AI 与职业相关的问题，而这桌缺一个刚开始学习的人。',
      }),
      table('idea-to-product', 'workshop', '一个想法怎样变成产品？', 3, '还缺一个把产品做出来的人', formingTable),
      table('creative-drive', 'workshop', '怎样重新找回创造欲？', 3, '还缺一个重新开始创作的人', formingTable),
    ],
  },
]
